...
async function loadReading() {
    ...
    if (!id) {
        document.body.innerHTML = '<p class="p-8 text-center text-red-600">Invalid reading material ID.</p>';
        return;
    }

    try {
        const res = await fetch(`/resources/reading/${encodeURIComponent(id)}`, { credentials: "include" });
        if (!res.ok) throw new Error("Reading material not found");
        const data = await res.json();

        // Header
        const cat = textOrEmpty(data.category) || "Uncategorized";
        const title = textOrEmpty(data.previewTitle || data.title);
        ...
        const heroUrl = textOrEmpty(data.heroImage || data.hero || data.hero_url || data.heroSrc);
        if (heroUrl) {
            heroEl.src = heroUrl;
        ...
        if (!sections.length) {
            ...
            empty.textContent = "No content available for this reading.";
        ...
        document.body.innerHTML = `
          <div class="max-w-xl mx-auto p-8 text-center">
            <p class="text-red-600 font-semibold">Failed to load reading material.</p>
...
document.addEventListener("DOMContentLoaded", loadReading);